\chapter{Intégration de la spéficication proposée dans le programme}\label{sec:Theme2}
...
\begin{figure}[h!]
\begin{center}
  \includegraphics[height=0.4\textheight]{buchi_auto.png}
\end{center}
...
Nous avons utilisé les scénarios de tests suivant lors de nos expérimentations.
Nous nous sommes concentré sur des situations impliquant des problèmes de
concurrences, et en particulier sur des erreurs liées à une absence ou une
...
dans chaque batterie, elle est decrémentée puis remise à sa valeur maximale
après avoir atteint la valeur zéro. Dans ce test, on désire éviter que les deux
...
verte. Ils sont synchronisé à l'aide de mutex. Dans ce premier cas, on veut
vérifier que les deux feux ne sont jamais verts de manière simultanée.

\paragraph{crossing\_exclusive\_mutex}
(88 lignes, 2 threads)
...
ensemble de proposition LTL classiques et fréquemment utilisée.
...
requête par le client est bien systématiquement suivi d'une réponse du serveur.
...
Une situation de producteur / consommateur, avec un seul producteur et un seul
consommateur, qui incrémentent et décrémentent une variable partagée.
Dans ce cas de figure, on tente de synchroniser le système sans primitives de
synchronisation. On vérifie que on ne consomme jamais plus d'éléments qu'il n'y
a eu de production.

\paragraph{prod\_cons\_mutex}
...
variables partagées. On tente de prouver l'état final atteint si un certaine
situation intermédiaire est atteinte, ce qui donne l'opportunité de manipuler
une formule LTL plus complexe que dans les tests précédent.
