...
Le pouvoir d'expressivité de \ac{LTL} et \ac{CTL} n'est ni équivalent, ni même comparable.
En effet, \ac{LTL} ne permet pas de quantifier existentiellement : une formule \ac{CTL}
utilisant une quantification existentielle n'a donc pas d'équivalent en \ac{LTL}. La
réciproque peut être présentée à l'aide de la propriété \ac{LTL} \(F (p \land X p)\)
(pour toutes les traces, on atteint un état vérifiant \(p\) et dont le
successeur vérifie \(p\)) n'a pas d'équivalent dans \ac{CTL}. La formule \ac{CTL} \(AF (p
...
Par la suite, nous allons nous concentrer sur \ac{LTL}, car cette logique est
...
La définition de \ac{LTL} ajoute deux opérateurs temporels à la logique
...
(\ac{BDDs}, analyse de valeurs, analyse d'intervalles\dots). CPAchecker est capable
