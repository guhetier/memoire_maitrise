...
A partir de la représentation sous forme de BDD, on peut évaluer la
...
SymDivine\cite{SymDivine} permet la vérification de propriétés LTL sur du
...
à vérifier\cite{abstrate_state_graph, dsilva_survey_2008}.
...
lorsque \lstinline!x = 0! pourrait ici correspondre à une division par
...
\end{minipage}\hfill
\begin{minipage}{.45\textwidth}
\begin{lstlisting}[language=C, caption=Programme booléen,frame=tlrb,
...
Counter-Exemple Guided Abstraction Raffinement (CEGAR)\cite{dsilva_survey_2008}.

\subsubsection{Counter-Example Guided Abstraction Raffinement (CEGAR)}

La méthode CEGAR permet de construire une abstraction en itérant quatre phases :
...
\item
  ...
  trouver la trace L1, L2, L3, L5, L6 qui mène à une erreur dans l'abstraction.
\item
  \textbf{Simulation :} Une simulation symbolique permet de constater que la
  trace n'est pas réalisable. En effet, \lstinline!x! est non-nul lors
  du test à \lstinline!L5! donc la transition de \lstinline!L5! vers
  \lstinline!L6! n'est pas réalisable.
\item
  ...
  prédicat \lstinline!{x = 0}! indiquant si \lstinline!x! est nul est ajouté. On
  obtient alors l'abstraction plus précise du listing \ref{lst:CEGAR_2}.
\item
  \textbf{Vérification :} Le model-checker trouve cette fois l'exécution L1, L2,
  L3, L4, L5, L6.
\item
  \textbf{Simulation :} cette trace est encore une fois incorrecte, puisque
  \lstinline!y! ne vaut pas \lstinline!1! en L3.
\item
  \textbf{Raffinement :} un prédicat \lstinline!{y = 1}! est ajouté. On obtient
...
\begin{lstlisting}[language=C, label=lst:CEGAR_1]
L1: ;
L2: ;
L3: if (*)
L4:     ;
...
L6:    assert(false);
\end{lstlisting}
\begin{lstlisting}[language=C, label=lst:CEGAR_2]
L1: {x = 0} = {false};
L2: ;
L3: if (*)
L4:     {x = 0} = true;
...
L6:    assert(false);
\end{lstlisting}
\begin{lstlisting}[language=C, label=lst:CEGAR_3]
L1: {x = 0} = false;
L2: {y = 1} = false;
L3: if ({y = 1})
L4:     {x = 0} = true;
...
CEGAR, puisque un améliorer le choix des nouveaux prédicats peut éviter des
...
Les méthodes d'abstraction par prédicat, dont l'algorithme CEGAR, se basent sur
...
l'analyse du programme booléen. C'est plus vrai encore pour l'algorithme CEGAR,
...
\item
  ...
  l'algorithme CEGAR\cite{predicate_abstraction_over_approximation}.
...
La plupart des model-checkers utilisant la méthode CEGAR basent leurs procédures
...
CEGAR. Cependant, il utilise des techniques basées sur des raisonnements de type
...
(BDDs, analyse de valeurs, analyse d'intervalles\dots). CPAchecker est capable
...
permet de vérifier des propriétés LTL.
...
Partant de la forme SSA, la relation de transition se déplie facilement
